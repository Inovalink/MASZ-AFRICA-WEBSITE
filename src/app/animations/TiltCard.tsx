...
interface TiltCardProps {
  imageSrc: string;
  title: string;
  bottomText?: string; // now optional
  className?: string;
}

export default function TiltCard({ imageSrc, title, bottomText, className = '' }: TiltCardProps) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const rotateX = -(y - centerY) / 50; // subtle tilt
  ...
  return (
    ...
    >
      <div ref={innerRef} className="absolute inset-0  w-full h-full" style={{ transformStyle: 'preserve-3d' }}>
        ...
        {/* Only render text if bottomText exists */}
        {bottomText && (
          <div
            ref={textRef}
